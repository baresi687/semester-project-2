...
export function listingFeedHtml(arr) {
  return arr.map(({ id, media, title, bids }) => {
    ...
    let titleOfListing = title ? title : 'No Title';
    ...
    return `<a href="listing-details.html?id=${id}" class="group">
              <div class="flex flex-col gap-4">
                <img src="${listingImg}" alt="${escapedLessThanTitle}" class="object-cover h-72 rounded sm:h-48 lg:h-56">
                <h3 class="text-xl font-medium capitalize whitespace-nowrap overflow-hidden text-ellipsis">${escapedLessThanTitle}</h3>
